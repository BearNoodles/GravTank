		...
		if(panel_enabled_[panel_index])
		{
			...
			if(num_touch_datasets)
			{
				...
				for (Int32 touch_report_index = 0; touch_report_index < data_read->reportNum; ++touch_report_index)
				{
					...
					// check to see if touch has been processed
					bool is_new_touch = true;

					// go through all touches and see if this is an existing touch
					for (Int32 touch_index = 0; touch_index < SCE_TOUCH_MAX_REPORT; ++touch_index)
					{
						if (new_touch[touch_index] == false)
						{
							Touch* touch = &touches[touch_index];
							if (touch_report->id == touch->id)
							{
								// alread processed this touch so it's not new
								is_new_touch = false;
								break;
							}
						}
